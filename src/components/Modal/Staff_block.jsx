...
export default function Block_staff({
  isOpen,
  onClose,
  staffData,
  SetOpenBlock,
  ...
  const delete_staff = async (id) => {
    try {
      const response = await userInstance.delete(`/admin/staff/${id}`);
      SetOpenBlock(false);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <Dialog open={isOpen} onClose={onClose} className="relative z-10">
      <DialogBackdrop
        transition
        className="fixed inset-0 bg-black bg-opacity-10 transition-opacity data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in"
      />
